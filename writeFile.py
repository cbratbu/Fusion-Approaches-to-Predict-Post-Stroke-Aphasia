...
def writeResults(predictionModel):
    """Writes the top-3 best prediction outputs in each of the top-k feature setting 
    
    """
    output_file = output_file_path + predictionModel + "_final.csv"
    with open(output_file,"w", newline='') as csvfile:
        ...
        for dataFolder in dataFolders:
            results_folder = PATH + "results/" + dataFolder + "/" + predictionModel + "_predictions" + "/model_performances"
...
def writeFiles(predictionModel):
    files = os.listdir(PATH + "results")
    dataFolders = [f for f in files if not os.path.isfile(PATH + "results/" + f)]    
    output_file = output_file_path + predictionModel + "_final.csv"
    ...
    for source in dataFolders:
        ...
        data.to_csv(PATH+"results/" + source + "/"+ predictionModel + "_predictions"+ "/"+ source + "_aggregate.csv")
        
        
def modelWiseUpdate(predictionModels):
    for predictionModel in predictionModels:
        writeResults(predictionModel)
        writeFiles(predictionModel)
    
    
if __name__ == "__main__":
    predictionModels = parameters["-model"]
    modelWiseUpdate(predictionModels)
